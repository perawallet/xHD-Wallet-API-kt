...
class XHDWalletAPITest {
    ...
    inner class KeyGenTests {
        ...
        @Test
        fun hardenTest() {
            assert(XHDWalletAPIBase.harden(0u) == 2147483648u) {
                "harden(0) and 2147483648 are not equal"
            }
            assert(XHDWalletAPIBase.harden(1u) == 2147483649u) {
                "harden(1) and 2147483648 are not equal"
            }
            assert(XHDWalletAPIBase.harden(44u) == 2147483692u) {
                "harden(44) and 2147483648 are not equal"
            }
            assert(XHDWalletAPIBase.harden(283u) == 2147483931u) {
        ...
        fun bip44PathTest() {
            val addressPath =
                XHDWalletAPIBase.getBIP44PathFromContext(
                    KeyContext.Address,
                    0u,
                    0u,
                    0u
        ...
        fun deriveHardenedTest() {
            ...
            val index = XHDWalletAPIBase.harden(44u)
        ...
        fun fromSeedTest() {
            ...
            val output = XHDWalletAPIBase.fromSeed(seed)
        ...
        fun deriveChildNodePrivateTest() {
            val index = XHDWalletAPIBase.harden(283u)
            val extendedKey =
                helperStringToByteArray(
        ...
        fun deriveChildNodePublicTest() {
            ...
            val outputP =
                c.deriveChildNodePublic(rootKey, 0u, Bip32DerivationType.Peikert)
        ...
        fun fromSeedBip39Test() {
            ...
            val rootKey = XHDWalletAPIBase.fromSeed(seed.toSeed())
        ...
        fun derivePubliclyChildrenTest() {
            ...
            val bip44Path =
                listOf(
                    ...
                    XHDWalletAPIBase.harden(account),
                    0u,
                )
        ...
        fun checkDerivationTypeSafeLevelsTest() {
            ...
            val s =
                object : XHDWalletAPIJVM(seed.toSeed()) {
            ...
            val extendedKey = XHDWalletAPIBase.fromSeed(seed.toSeed())
            ...
            // little Endianess
            extendedKey[0] =
                (extendedKey[0].toInt() and 0b11111000)
                    .toByte() // the lowest 3 bits of the first
            // bits of kL are cleared
            extendedKey[31] =
                (extendedKey[31].toInt() and 0b01111111)
                    .toByte() // the highest bit of the last
            // bit is cleared
            extendedKey[31] =
                (extendedKey[31].toInt() or 0b01000000)
                    .toByte() // the second highest bit of the
            // last bit is set
            ...
            val matrix =
                mutableListOf(
                    mapOf("type" to Bip32DerivationType.Peikert, "value" to 8)
                )

            if (khovratovichSafetyTestFlag) {
                matrix.add(
                    mapOf(
                        "type" to Bip32DerivationType.Khovratovich,
                        "value" to 67108864
                    )
                )
            }
        ...
        fun deriveMaxLevelsForKnownSeedTest() {
            // Tests the maximum number of levels of derivation for a seed whose max
            // levels we know
            val seed =
                MnemonicCode(
                    "salon zoo engage submit smile frost later decide wing sight chaos renew lizard rely canal coral scene hobby scare step bus leaf tobacco slice".toCharArray()
    ...
    inner class ValidateDataTests {
        ...
        fun validateNonceDataTest() {
            ...
            val valid = XHDWalletAPIBase.validateData(challenge.toByteArray(), metadata)
        ...
        fun validateMsgTest() {
            ...
            val valid = XHDWalletAPIBase.validateData(message.toByteArray(), metadata)
            assert(valid) { "validation failed, message not in line with schema" }
        }
        ...
        fun validateMsgBase64Test() {
            ...
            val valid =
                XHDWalletAPIBase.validateData(
                    Base64.getEncoder().encode(message.toByteArray()),
                    metadata
                )
            assert(valid) { "validation failed, message not in line with schema" }
        }
        ...
        fun validateMsgPackTest() {
            ...
            val valid = XHDWalletAPIBase.validateData(message, metadata)
            assert(valid) { "validation failed, message not in line with schema" }
        }
        ...
        fun passThroughIllegalPrependFailedTest() {
            ...
            try {
                for (prefix in XHDWalletAPIBase.prefixes) {
                    XHDWalletAPIBase.validateData(
    ...
    inner class ECDHTests {
        ...
        fun encryptDecryptECDHTest() {
            ...
            // Decrypt
            val plaintext =
                ls.cryptoSecretBoxOpenEasy(ciphertext, nonce, aliceSharedSecret)
