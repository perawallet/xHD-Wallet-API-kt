...
const val ED25519_SCALAR_SIZE = 32
const val ED25519_POINT_SIZE = 32
...
abstract class XHDWalletAPIBase(private var seed: ByteArray) {
  ...
  fun rawSign(
    ...
    val r = this.safeModQ(MessageDigest.getInstance("SHA-512").digest(c + data))

    // \(4):  R = r * G (base point, no clamp)
    val R = lazySodium.cryptoScalarMultEd25519BaseNoclamp(r).toBytes()

    val h = this.safeModQ(MessageDigest.getInstance("SHA-512").digest(R + publicKey + data))
